...
function primary(text){
  Toastify_exc({ text: text, style: { background: '#007bff' }});
}

function warning(text){
  Toastify_exc({ text: text, style: { background: '#ffc107', color: 'black' }});
}

function success(text){
  Toastify_exc({ text: text, style: { background: '#28a745', 'border-radius': '5px' }});
}

function danger(text){
  Toastify_exc({ text: text, style: { background: '#dc3545' }});
}
