...
function MuteOrUnmuteMic() {
    try {
        DC_RPC_Client.getVoiceSettings()
            .then((vs) => {
                DC_RPC_Client.request('SET_VOICE_SETTINGS', {
                    mute: vs.mute ? false : true
                });
            })
            .catch((e) => {
                console.log(e);
    ...
    try {
        DC_RPC_Client.getVoiceSettings()
            .then((vs) => {
                ...
                DC_RPC_Client.request('SET_VOICE_SETTINGS', object);
            })
            .catch((e) => {
                console.log(e);
            });
...
async function getAvailableAudioDevices(client) {
  try {
    const devices = await client.request('GET_VOICE_SETTINGS');
