...
class MainScreen {
  ...
  startAllHandleMessages(){
    ...
    ObsService.Disconnected((res)=>{
      if(this.ObsWebSocketStarted == true){
        this.Notification(null, translator.getNameTd('.obswssdesconnectdromdss'));
  ...
  startServices(){
    if(DAO.OBS.get('StartOnApp') == true)
      Start_obs_wss(this);
  }
}
  ...
  if(screen.ObsWebSocketStarted != true){
    ...
    if(Ip_OBS != null && Port_OBS != null){
      ObsService.Connect(Ip_OBS, Port_OBS, Pass_OBS).then((res)=>{
        ...
        screen.Notification(null, translator.getNameTd('.obswssconneted'));
